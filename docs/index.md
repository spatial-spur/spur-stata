...
transformation stage of the workflow. For inference, it is designed to work
alongside the `scpc` package by Müller and Watson.
...
Unlike the R and Python packages, Stata does not currently expose a one-call
pipeline wrapper for the full SPUR workflow. The intended usage is the command
sequence shown above: `spurtest`, branch choice, `spurtransform` if needed,
regression, then `scpc`.
